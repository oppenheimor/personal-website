...
1. 专用 Chrome profile：只给内容工厂用，单独保存登录态。
2. CDP Proxy：常驻在本机，把 Chrome 暴露成 `web-access` 可调用的 HTTP API。

日常 Chrome 继续日常用。自动化 Chrome 专门跑任务。两个世界隔开，心里会踏实很多。
...
这样 Chrome 的调试端口更稳定，也不会依赖 `chrome://inspect` 里那个手动勾选开关。
...
```json
{
  "Browser": "Chrome/149.0.x",
  "Protocol-Version": "1.3",
  "webSocketDebuggerUrl": "ws://127.0.0.1:9222/devtools/browser/<uuid>"
}
```

这里最重要的是 `webSocketDebuggerUrl`。如果这个接口返回 404，通常说明你还在用 `chrome://inspect` 勾选模式，或者启动的不是这套专用 Chrome。
...
如果平台登录态过期，就打开那套专用 Chrome，手动登录一次。不要把 Cookie 复制出来，也不要让 Agent 记住验证码。该人工的地方就人工，省得以后踩更大的坑。
...
如果 `lsof` 没结果，说明专用 Chrome 没起来。重启：

```bash
launchctl kickstart -k gui/$(id -u)/com.hermes.chrome-xhs
```

如果 `lsof` 有结果，但 `/json/version` 是 404，大概率不是命令行启动的专用 Chrome，而是又回到了 `chrome://inspect` 勾选模式。
...
如果页面标题能读到，但正文是登录页，那就是平台登录态过期。去专用 Chrome 里手动登录，不要试图把验证码自动化。
...
临时网页操作可以接管日常 Chrome。定时任务不行。定时任务需要自己的浏览器、自己的 profile、自己的常驻进程，还要有一套能快速判断问题在哪儿的检查命令。

最后这套结构很简单：

```text
专用 Chrome profile 保存登录态
